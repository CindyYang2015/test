 ...
 * Package Name:cn.cloudwalk.common.cache
 * Date:2016年3月29日下午6:53:36
 * Copyright @ 2010-2016 Cloudwalk Information Technology Co.Ltd All Rights Reserved.
 *
*/

package com.cloudwalk.common.cache;
...
import com.cloudwalk.common.spring.SpringContextHolder;
...
/**
 * ClassName:JedisUtils <br/>
 * Description: Redius缓存. <br/>
 * Date: 2016年3月29日 下午6:53:36 <br/>
 *
 * @author 陈腾
 * @version 1.0.0
 * @since JDK 1.7
...
public class JedisUtils {
  ...
  /**
   * mapObjectExists:判断Map缓存中的Key是否存在. <br/>
   *
   * @param key 键
   * @param mapKey 值
   * @return boolean
  ...
  public static long setObjectList(String key, List<?> value, int cacheSeconds) {
    ...
    try {
      ...
      byte[][] byteValue = new byte[list.size()][];
      list.toArray(byteValue);
      result = jedis.rpush(getBytesKey(key), byteValue);
      if (cacheSeconds != 0) {
        jedis.expire(key, cacheSeconds);
      }
  ...
  public static String setObjectMap(String key, Map<String, ?> value, int cacheSeconds) {
    ...
    try {
      ...
      for (Map.Entry<String, ?> e : value.entrySet()) {
    	byte[]bytes = toBytes(e.getValue());
    	if(bytes != null) {
            map.put(getBytesKey(e.getKey()), bytes);
    	}
      }
      result = jedis.hmset(getBytesKey(key), map);
      if (cacheSeconds != 0) {
  ...
  public static long setObjectSet(String key, Set<?> value, int cacheSeconds) {
    ...
    try {
      ...
      byte[][] bytevalue = new byte[set.size()][];
      set.toArray(bytevalue);
      result = jedis.sadd(getBytesKey(key), bytevalue);
      if (cacheSeconds != 0) {
        jedis.expire(key, cacheSeconds);
      }
