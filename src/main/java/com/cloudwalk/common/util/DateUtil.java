/**
 * Project Name:sys-mng
 * File Name:DateUtil.java
 * Package Name:com.linc.usermanager.util
 * Date:2014年11月21日下午3:22:48
...
public class DateUtil {
    ...
    public static int getYear(Date date) {
      ...
      aCal.setTime(date);
      return aCal.get(Calendar.YEAR);
    }
    
    /**
     * getMonth 根据Date时间获取月份
     *
     * @param date
     * @return int 返回的月份
     */
    public static int getMonth(Date date) {
      Calendar aCal = GregorianCalendar.getInstance();
      aCal.setTime(date);
      return aCal.get(Calendar.MONTH) + 1;
    }
     ...
     * @throws Exception 
     */
    public static Date getPreDaysDate(Date date,int day) throws Exception {  
    	if(date == null) return null;    	
    	Calendar date2 = Calendar.getInstance();
		date2.setTime(date);
		date2.set(Calendar.DATE, date2.get(Calendar.DATE) - day);
		SimpleDateFormat dateFm = new SimpleDateFormat("yyyy/MM/dd"); //格式化当前系统日期
		String dateTime = dateFm.format(date2.getTime());			
		return dateFm.parse(dateTime);    	
    }
	...
	private static final SimpleDateFormat printFormat = new SimpleDateFormat(PRINT_FORMAT);

	private static final SimpleDateFormat dateFormat = new SimpleDateFormat(DATE_FORMAT_1);

	private static final SimpleDateFormat timeFormat = new SimpleDateFormat(TIME_FORMAT);

	private static final SimpleDateFormat loginFormat = new SimpleDateFormat(LOGIN_FORMAT);
}
